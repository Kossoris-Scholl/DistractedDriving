...
def tf_run(balanced=False):
    ...
    if balanced:
        ...
        g = df.groupby(['Stimulus', 'Anti.Stim'])
    ...
    with tf.Session() as sess:
        ...
        results['accuracy'] = accuracy.eval({x: X_test, y: Y_test})
        print("Accuracy:", results['accuracy'])
        results['mse'] = 1 - results['accuracy']
        print('Mean squared error: ', results['mse'])
        global result
        result = tf.argmax(pred, 1).eval({x: X_test, y: Y_test})
        results['mse'] = metrics.mean_squared_error(y_true=Y_test[['Anti.Stim']], y_pred=correct_prediction)
...
def main():
    ...
    unbalanced = tf_run(False)
    print("\n--------------------- After balance ------------------------\n")
    balanced = tf_run(True)
